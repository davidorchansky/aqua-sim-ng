...
AquaSimMultiPathSignalCache::ReflCoeff(double theta, double s, double s_bottom)
{
  ...
  if (s>s_bottom) {
    std::complex<double> complex (acos(s/s_bottom));
    thetac = std::real(complex);
  } else {
    thetac = acos(s/s_bottom);
  }
  ...
  if (theta<thetac) {
    if (thetac==0) return -1;
      ...
      std::complex<double> complex (exp(sqrt(-1) * pi * (1-theta/thetac)));
      return std::real(complex);
    }
  }
  //theta>=thetac
  x1=rho2/s*sin(theta);
