...
RevQueues::ClearExpired(Time ExpireTime)
{
  ...
  while( Head_ != NULL && Head_->EndTime.GetSeconds() < ExpireTime.GetSeconds() + BACKOFF_DELAY_ERROR ) {
      tmp = Head_;
      Head_ = Head_->next;
      delete tmp;
  }
...
RevQueues::UpdateStatus(int RevID, RevType new_type)
{
  ...
  while( pos != NULL ) {
    if( pos->RevID == RevID ) {
      ...
      if( send_time.GetSeconds() < 0.0 ) {
	  NS_LOG_WARN("UpdateStatus: handshake time takes too long, cancel sending");
	  DeleteRev(RevID);
	  return;
      }

      if( send_time.GetSeconds() > 0.0 && pos->m_sendTimer != NULL) {
